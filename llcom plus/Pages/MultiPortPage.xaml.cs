...
namespace llcom_plus.Pages
{
    ...
    public partial class MultiPortPage : Page
    {
        ...
        public MultiPortPage(int slotCount, bool showSlotSendPanel = true)
        {
            ...
            ExternalOptionsPanel.Visibility = showSlotSendPanel ? Visibility.Collapsed : Visibility.Visible;
        }
        ...
        {
            CloseAll(closeMainUart: true, detachMainUart: true);
        }
        ...
        private void CloseAll(bool closeMainUart = true, bool detachMainUart = false)
        {
            foreach (var slot in slots)
            {
                slot.Close(closeMainUart);
                if (detachMainUart)
        ...
        private sealed class PortSlot
        {
            ...
            {
                var selected = NormalizePortName(portComboBox.Text);
            ...
            public void SetPortName(string portName)
            {
                ...
                if (useMainUart && !string.IsNullOrWhiteSpace(normalizedPortName))
            ...
            public void Close(bool closeMainUart = true)
            {
            ...
            private Border BuildView()
            {
                var root = new Border
                {
                    ...
                    BorderBrush = new SolidColorBrush(Color.FromRgb(210, 222, 232)),
            ...
            {
                var port = IsOpen ? PortName : NormalizePortName(portComboBox.Text);
            ...
            private bool Open()
            {
                var portName = NormalizePortName(portComboBox.Text);
                if (string.IsNullOrWhiteSpace(portName))
                    return false;
            ...
            private void WriteDataLog(string direction, byte[] data, bool hex, bool sent, bool updateCounters = true, bool writeSessionLog = true)
            {
                ...
                AppendLog(direction, FormatData(data, hex), sent ? ResourceBrush("AppDataSentBrush", Brushes.DarkRed) : ResourceBrush("AppDataReceivedBrush", Brushes.DarkGreen));
            }
            ...
            private void AppendLog(string direction, string text, Brush dataBrush)
            {
                ...
                paragraph.Inlines.Add(new Run(linePrefix) { Foreground = ResourceBrush("AppGlassMutedBrush", Brushes.Gray) });
            ...
            private static Brush GetLogBrush(string direction)
            {
                ...
                {
                    case "TX":
                        return ResourceBrush("AppDataSentBrush", Brushes.DarkRed);
                    case "RX":
                        return ResourceBrush("AppDataReceivedBrush", Brushes.DarkGreen);
                    case "ERR":
                        return Brushes.Red;
                    default:
                        return ResourceBrush("AppGlassTextBrush", Brushes.Black);
                }
            }

            private static SolidColorBrush ResourceBrush(string key, SolidColorBrush fallback)
            {
                return Application.Current?.Resources[key] as SolidColorBrush ?? fallback;
            }
