...
if ($PSVersionTable.PSVersion.Major -ne 5 -or [Environment]::Is64BitProcess -ne ($Platform -eq 'x64')) {
    throw 'Use matching-bitness Windows PowerShell 5.1. This test never builds or opens an application window.'
}
...
$testName = 'llcom-workspaces-' + [Guid]::NewGuid().ToString('N')
...
try {
    ...
    Add-Type -Path (Join-Path $PSScriptRoot 'WorkspaceProbe.cs')
    [WorkspaceProbe]::Run($assembly, $testRoot)
    Write-Host 'PASS  All 13 workspace safety / restore checks.'
}
finally {
    ...
    if ([IO.Path]::GetDirectoryName($resolvedRoot) -ne $tempBase -or [IO.Path]::GetFileName($resolvedRoot) -ne $testName -or
        $testName -notmatch '^llcom-workspaces-[0-9a-f]{32}$') { throw 'Refusing cleanup outside the isolated test directory.' }
